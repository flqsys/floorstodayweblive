...
function ft_inbox_default_email_template() {
    return "<p><strong>New homepage lead received.</strong></p>\n"
        ...
        . "<tr><td style=\"padding:6px 12px;border-bottom:1px solid #eee;\"><strong>Preferred Visit Time</strong></td><td style=\"padding:6px 12px;border-bottom:1px solid #eee;\">{preferred_visit_time}</td></tr>\n"
...
function ft_inbox_sanitize_lead_payload($request) {
    ...
    return [
        ...
        'number_of_rooms' => sanitize_text_field($params['numberOfRooms'] ?? $params['number_of_rooms'] ?? ''),
        'start_time' => sanitize_text_field($params['startTime'] ?? $params['start_time'] ?? ''),
        'preferred_visit_time' => sanitize_text_field($params['preferredVisitTime'] ?? $params['preferred_visit_time'] ?? ''),
...
function ft_inbox_get_lead($lead_id) {
    ...
    foreach (['admin', 'client'] as $type) {
        $lead[$type . '_email_status']     = get_post_meta($post->ID, '_ft_inbox_' . $type . '_email_status', true);
        $lead[$type . '_email_error']      = get_post_meta($post->ID, '_ft_inbox_' . $type . '_email_error', true);
        $lead[$type . '_email_checked_at'] = get_post_meta($post->ID, '_ft_inbox_' . $type . '_email_checked_at', true);
    }
...
function ft_inbox_render_settings_page() {
    ...
    $field_descriptions = [
        ...
        'preferred_visit_time' => 'Preferred visit time (Morning / Afternoon / Evening)',
