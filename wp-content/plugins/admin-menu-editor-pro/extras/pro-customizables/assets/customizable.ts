...
export namespace AmeCustomizable {
	...
	export type SettingChangeListener = (setting: Setting, newValue: any) => void;
...
export namespace AmeCustomizableViewModel {
	...
	export class SimpleVm extends ThrottledPreviewRegistry implements CustomizableVmInterface {
		...
		/**
		 * Update registered settings with new values.
		 *
		 * The newValueCallback will be called for each setting ID, and it should return the new
		 * value for that setting. If it returns None, the setting will be left unchanged.
		 */
		updateSettings(newValueCallback: (settingId: string) => Option<unknown>) {
			for (const settingId of this.settings.getAllSettingIds()) {
				lift(
					[this.settings.get(settingId), newValueCallback(settingId)],
					(setting, newValue) => setting.value(newValue)
				);
			}
		}

		addSettingChangeListener(listener: AmeCustomizable.SettingChangeListener): symbol {
			return this.settings.addChangeListener(listener);
		}
	}
