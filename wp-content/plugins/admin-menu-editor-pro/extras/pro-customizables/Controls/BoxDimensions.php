...
class BoxDimensions extends AbstractNumericControl {
	...
	protected static int $generatedIdCounter = 0;
	...
	public function __construct($settings = [], $params = [], $children = []) {
		...
		} else if ( $this->mainBinding instanceof AbstractStructSetting ) {
			//Detect available dimensions.
			$dimensionNames = [];
			foreach (self::DEFAULT_DIMENSION_NAMES as $key => $name) {
				$child = $this->mainBinding->getChild($key);
				if ( $child ) {
					$dimensionNames[$key] = $name;
				}
			}
	...
	public function renderContent(Renderer $renderer, Context $context) {
		if ( !($this->mainBinding instanceof AbstractStructSetting) ) {
			throw new \InvalidArgumentException('The main setting for this control must be a struct setting.');
		}
		if ( !($this->mainBinding instanceof HasUnit) ) {
			throw new \InvalidArgumentException('The main setting for this control must implement HasUnit.');
		}
	 ...
	 */
	protected function getDimensionNames(): array {
