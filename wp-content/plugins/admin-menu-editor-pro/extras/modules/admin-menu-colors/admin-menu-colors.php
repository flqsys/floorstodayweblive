	...
	private function generateMenuColorStyle($currentSettings, $customMenu, $globalGenerator = null) {
		...
		if ( !empty($customMenu['tree']) ) {
			...
			foreach ($customMenu['tree'] as &$item) {
				...
				if ( $subType === 'heading' ) {
					//Note: The selector is intentionally doubled to override styles generated by
					//MenuHeadingStyler, which also uses a doubled selector.
					$extraSelectors = ['.ame-menu-heading-item', '.ame-menu-heading-item'];
				} else {
					$extraSelectors = [];
				}
