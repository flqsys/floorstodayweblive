...
(function ($) {
	...
	function restoreCollapsedHeadings() {
		...
		for (let id in previouslyCollapsedHeadings) {
			...
			const $heading = $adminMenu.find('#' + id + '.ame-collapsible-heading');
			if ($heading.length > 0) {
	...
	jQuery(function ($: JQueryStatic) {
		...
		if (wsAmeProAdminHelperData.setHeadingHoverColor && (typeof Array.prototype.map !== 'undefined')) {
			...
			const $adminCssNode = $('link#admin-menu-css').first(); //Base admin menu stylesheet.
			const $colorsCssNode = $('link#colors-css').first();    //Current admin color scheme.
			let insertTarget: JQuery | null = null;
			if (($adminCssNode.length > 0) || ($colorsCssNode.length > 0)) {
				//Append the new style after whichever of the above comes last, so that it
				//has the best chance of overriding WP's default menu styles.
				if (($adminCssNode.length > 0) && ($colorsCssNode.length > 0)) {
					insertTarget = ($adminCssNode[0].compareDocumentPosition($colorsCssNode[0]) & Node.DOCUMENT_POSITION_FOLLOWING)
						? $colorsCssNode
						: $adminCssNode;
				} else if ($adminCssNode.length > 0) {
					insertTarget = $adminCssNode;
				} else if ($colorsCssNode.length > 0) {
					insertTarget = $colorsCssNode;
				}
			}

			if (insertTarget) {
				$newStyle.insertAfter(insertTarget);
			} else {
