...
use YahnisElsts\AdminMenuEditor\MenuHeadingStyles\MenuHeadingStyler;
...
class wsMenuEditorExtras {
	...
	private ?MenuHeadingStyler $menuHeadingStyler = null;
	...
	function __construct($wp_menu_editor){
		...
		if ( (is_admin() || (defined('DOING_AJAX') && DOING_AJAX)) && version_compare(PHP_VERSION, '5.6', '>=') ) {
			require_once AME_ROOT_DIR . '/extras/menu-headings/menu-heading-styles.php';
			$this->menuHeadingStyler = new MenuHeadingStyler($this->wp_menu_editor);
		}
		...
		if (isset(
			...
			if ( stripos($title, '<span') !== false ) {
				...
				if ( @$dom->loadHTML($title) ) {
					...
					$result = $xpath->query('//span[contains(@class,"update-plugins") or contains(@class,"awaiting-mod") or contains(@class,"menu-counter")]');
					if ( $result->length > 0 ) {
						$span = $result->item(0);
...
wsEditorData.importMenuNonce = "<?php echo esc_js(wp_create_nonce('import_custom_menu'));  ?>";
	...
	function add_extra_buttons(){
		...
		<input type="button" id='ws_edit_heading_styles_next' value="Headings" class="button ws_main_button" title="Edit the appearance of menu headings" />
	...
	public function add_extra_scripts($baseDeps) {
		...
		$extras = [
			...
			//This is obsolete as of 2026-04 because a new heading styler has been implemented.
			//It should be removed in a future version, once it's confirmed that the new styler
   ...
   */
	...
	/**
	 * Format a menu heading.
	 *
	 * @param array $item
	 * @return array
	 */
	private function create_menu_heading(array $item): array {
		$item['is_unvisitable'] = true;
		if ( !$this->menuHeadingStyler ) {
			//This should never happen. The heading styler should be initialized at this point.
			$item['menu_title'] = $item['menu_title'] . ' [ERR]';
			return $item;
		}

		return $this->menuHeadingStyler->createHeadingMenuItem($item);
	}
	...
	public function apply_admin_menu_filters($item, $parent_file = null) {
		...
		if ( ameMenuItem::get($item, 'sub_type') === 'heading' ) {
			$item = $this->create_menu_heading($item);
		}
	...
	public function enqueue_dashboard_deps() {
		...
		if ( !empty($config_id) ) {
			...
			$headingTextColor = ameUtils::get($custom_menu, ['menu_headings', 'titles', 'colors', 'textBase']);
			$headingTextHoverColor = ameUtils::get($custom_menu, ['menu_headings', 'titles', 'colors', 'textHover']);

			if ( empty($headingTextColor) && empty($headingTextHoverColor) ) {
				$is_helper_needed = true;
	...
	public function add_fa_selector_tab($tabs) {
		$tabId = 'ws_fontawesome_icons_tab';
		$tabTitle = 'Font Awesome';
		if ($tabs instanceof ameOrderedMap) {
			$tabs->insertBefore('ws_svg_icon_tab', $tabId, $tabTitle);
		} else {
			$tabs[$tabId] = $tabTitle;
		}
		return $tabs;
	...
	public function filter_available_modules($modules) {
		...
		$modules['tweaks'] = array(
			...
			'className'    => 'YahnisElsts\\AdminMenuEditor\\Tweaks\\Pro\\TweakManagerPro',
			'title'        => 'Tweaks',
