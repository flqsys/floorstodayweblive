	...
	if ($lrdata['action'] === "lrgawidget_hideShowWidget"){
		...
		switch ($lrdata['action']) {
			case "lrgawidget_getPermissions":
				...
				break;	
			case "lrgawidget_setPermissions":
				if (in_array("perm", DataStore::$RUNTIME["permissions"])){$call->setCurrentBlogRolesPermissions($lrdata['lrperms']);}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}
				break;
			default:
		...
		if (in_array("daterange", DataStore::$RUNTIME["permissions"])){
		...
		switch ($lrdata['action']) {
			...
			case "lrgawidget_getGraphData":
				...
				break;				
			case "lrgawidget_getCountries":
				if (in_array("countries", DataStore::$RUNTIME["permissions"])){$call->getCountries();}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}
				break;
			case "lrgawidget_getRealTime":
				if (in_array("realtime", DataStore::$RUNTIME["permissions"])){$call->getRealTime();}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}
				break;			
			case "lrgawidget_getKeywords":
				if (in_array("keywords", DataStore::$RUNTIME["permissions"])){$call->getKeywords();}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}	
				break;
			case "lrgawidget_getSources":
				if (in_array("sources", DataStore::$RUNTIME["permissions"])){$call->getSources();}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}	
				break;
			case "lrgawidget_setGraphData":
				if (in_array("graph_options", DataStore::$RUNTIME["permissions"])){$call->setGraphData($lrdata);}
				else{ ErrorHandler::FatalError(__('You do not have permission to access this tab!', 'lara-google-analytics'));}
				break;
			default:
				ErrorHandler::FatalError(__('Fatal Error', 'lara-google-analytics'),__('Invalid Call', 'lara-google-analytics'));
