...
class lrga_wp_woo_plugin extends lrga_earnings_sales{
	...
	private function getOrders(){
		...
		$date = $this->graphData["settings"]["date"];
		if (!empty($date)  && in_array($date, array_keys($pluginSettings["date"]["options"])) ){
			$args[$date] = ''.$this->startDate->format('U').'...'.$this->endDate->format('U').'';
		}
		
		$status = $this->graphData["settings"]["status"];
		if (!empty($status)  && in_array($status, array_keys($pluginSettings["status"]["options"])) ){
			$args['status'] = $status;
		}		
	...
	private function setGraphSeriesArray(){
		if (!empty($this->graphData["filters"][$this->currentFilter])){
			$this->graphItemsIds = array_keys($this->graphData["filters"][$this->currentFilter]);
		}
		...
		switch ($this->currentFilter){
			case "all":
				...
				break;			
			case "categories":
				$items  = $this->getCategories($this->graphItemsIds);
				break;						
			case "products":
				$items  = $this->getProducts($this->graphItemsIds);
				break;
			case "types":
				$items  = $this->getProductTypes($this->graphItemsIds);
				break;				
		}
	...
	protected function getRawSeriesData(){
		...
		$date_function = "get_".$this->graphData["settings"]["date"];

		foreach ($orders as $order){
			$order_id    = $order->get_id();
			$order_total = $order->get_total();
			$cDate = new \DateTime($order->$date_function());
					...
					switch ($this->currentFilter){
						case "categories":
							...
							foreach ($product_categories as $product_category){
								$this->setRawData("categories", $product_category, $cDate, $product_quantity, $product_total);
							}
							break;						
						case "products":
							$this->setRawData("products", $product_id, $cDate, $product_quantity, $product_total);
							break;
						case "types":
							$product      = $item->get_product();
							$product_type = $product->get_type();
							$this->setRawData("types", $product_type, $cDate, $product_quantity, $product_total);
							break;				
					}				
