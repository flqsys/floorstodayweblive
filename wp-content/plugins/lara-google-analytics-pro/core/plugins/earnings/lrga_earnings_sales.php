...
class lrga_earnings_sales{
	...
	protected function initializeGraphData(){
		$graphData = DataStore::database_get("user_options", $this->pluginId.'_graphdata');
	...
	private function is_hexColor($str){
		return (bool) preg_match('/#([a-f0-9]{3}){1,2}\b/i', $str);
	}
	...
	public function setGraphData($graphData){
		...
		$finalGraphData['currentfilter'] = $this->defaultFilterID;
		if ( (!empty($graphData['currentfilter']))  && in_array($graphData['currentfilter'], array_keys($pluginFilters)) ){
			$finalGraphData['currentfilter'] =  $graphData['currentfilter'];
		}
		
		if (!empty($graphData["settings"]) && is_array($graphData["settings"])){
			foreach ($graphData["settings"] as $setting => $value){
				if(in_array($setting, array_keys($pluginSettings))){
					$finalGraphData["settings"][$setting] = $pluginSettings[$setting]["default"];
					if(in_array($value, array_keys($pluginSettings[$setting]["options"]))){
						$finalGraphData["settings"][$setting] = $value;
					}
				}
			}
		}
		
		if (!empty($graphData["filters"]) && is_array($graphData["filters"])){
			foreach ($graphData["filters"] as $filter => $items){
				if (in_array($filter, array_keys($pluginFilters))){
					foreach ($items as $item => $values){
						if ((!empty($values['status'])) && ($values['status'] === "on") && ($this->is_hexColor($values['color']))){
							$finalGraphData['filters'][$filter][$item] = array("on", $values['color']);
						}	
					}
				}
			}
		...
		DataStore::database_set("user_options", $this->pluginId.'_graphdata' , $finalGraphData);
	}	
