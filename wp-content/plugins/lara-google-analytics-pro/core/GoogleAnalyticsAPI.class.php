...
class GoogleAnalyticsAPI{
	...
	public function getRealTime($params=array()) {
		return $this->httpRequest(self::API_URL . "/" . $this->property_id . ":runRealtimeReport" , $params, "JSON_POST", array("Authorization: Bearer " . $this->access_token));		
	}	
	...
	public function getKeywords($doCall=false) {
		if (!empty($this->webmasterSiteUrl)){
			...
			if ($doCall === true ){
				return $this->httpRequest($callURL, $this->queryParams, "JSON_POST");
			}
		}else{ErrorHandler::FatalError($this->errorMessages['missingWebmasterSiteUrl']); }
	}
