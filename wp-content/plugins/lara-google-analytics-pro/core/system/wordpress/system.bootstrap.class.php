...
class SystemBootStrap {
    ...
    public static function getCurrentUserRoles(){
        $user_meta=get_userdata(self::$user_id);
        $user_roles=$user_meta->roles;
        if (current_user_can('setup_network') && !in_array("administrator", $user_roles)){
	...
	private static function get_data_from_database(){
		...
		$sql = $wpdb->prepare ( "SELECT `name`, `value` FROM  `".lrgawidget_plugin_table."`  WHERE `site_id` = %d AND `blog_id` = %d", array(self::$site_id, self::$blog_id ));
		$call = $wpdb->get_results( $sql , ARRAY_A );
	...
	private static function set_field_to_database($name, $array){
		...
		$sql    = $wpdb->prepare ( "SELECT `name`, `value` FROM  `".lrgawidget_plugin_table."`  WHERE `site_id` = %d AND `blog_id` = %d AND `name` = %s", array(self::$site_id, self::$blog_id,$name));
		...
		if (!empty($exists)){
			$wpdb->update( lrgawidget_plugin_table, array('value' => $value), array('site_id' => self::$site_id, 'blog_id' => self::$blog_id, 'name' => $name), array('%s'), array('%d','%d','%s'));
			...
			$wpdb->insert( lrgawidget_plugin_table, array( 'site_id' => self::$site_id, 'blog_id' => self::$blog_id, 'name' => $name, 'value' => $value), array('%d','%d','%s','%s'));
	...
	private static function init_user_options(){
		...
		$woocommerce_graphdata = get_user_option('lrgapro-graphData_woocommerce', self::$user_id);
		if ($woocommerce_graphdata !== false){ $user_options["woocommerce_graphdata"] = json_decode($woocommerce_graphdata, true);}
		delete_user_option(self::$user_id, 'lrgapro-graphData_woocommerce');
	...
	public static function init_database(){
		...
		$db["permissions"]    = $db_data["permissions"];
	...
	public static function commit_database($db){
		...
		if(isset($db["permissions"]) && is_array($db["permissions"])){
			self::set_field_to_database("permissions", $db["permissions"]);
		}	
	}
}
