	...
	public static function activate() {
		if ( version_compare( PHP_VERSION, '5.6', '<' ) ) {
			...
			if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", lrgawidget_plugin_table ) ) != lrgawidget_plugin_table ) {
				$wpdb->query("CREATE TABLE `".lrgawidget_plugin_table."` (`id` int(10) NOT NULL AUTO_INCREMENT, `site_id` int(10) NOT NULL, `blog_id` int(10) NOT NULL, `name` TEXT NOT NULL, `value` TEXT NOT NULL, PRIMARY KEY (`id`))");
